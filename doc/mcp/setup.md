...
2. The consumer's `bin/artisan.dart` (the standard scaffold from `dart run
   fluttersdk_artisan install`) lists `DuskArtisanProvider` in its provider factory list,
   or relies on the auto-discovered `lib/app/_plugins.g.dart` barrel populated by
   `dart run fluttersdk_artisan plugins:refresh`.
3. `dart run fluttersdk_artisan list` shows the dusk commands (`dusk:snap`, `dusk:tap`,
...
# Cursor (per-project)
dart run fluttersdk_artisan mcp:install --path .cursor/mcp.json
```
...
actionable error (`"VM Service unreachable: state.json missing"`) until you run `dart run
fluttersdk_artisan start`. Once the state file exists the next tool call lazy-reconnects.

**`dusk_*` tools missing from the catalog.** The provider is not registered. Verify by
running `dart run fluttersdk_artisan list` and confirming the `dusk:*` command block. If
the block is empty, add `DuskArtisanProvider.new` to the consumer's `artisanProviders`
list in `bin/artisan.dart`, or run `dart run fluttersdk_artisan plugins:refresh` to
