...
/// `[0.0.1] - 2026-05-20` release with the required subsections.
///
...
/// `[0.0.1] - 2026-05-20` release with Added / Test coverage / Known gaps /
...
void main() {
  group('CHANGELOG.md promotion to [0.0.1] - 2026-05-20', () {
    test('CHANGELOG.md exists at the package root', () {
      expect(fileExists('CHANGELOG.md'), isTrue);
    });

    test('contains exactly one heading "## [0.0.1] - 2026-05-20"', () {
      final String raw = loadFile('CHANGELOG.md');
      final RegExp heading =
          RegExp(r'^## \[0\.0\.1\] - 2026-05-20\s*$', multiLine: true);
      final Iterable<RegExpMatch> matches = heading.allMatches(raw);
      expect(
        matches.length,
        equals(1),
        reason:
            'Step 20 must promote [Unreleased] into a single dated [0.0.1] - 2026-05-20 heading; got ${matches.length}',
      );
    });
    ...
    test('still carries an "## [Unreleased]" heading above the 0.0.1 entry',
      ...
      final int releaseIndex =
          raw.indexOf(RegExp(r'^## \[0\.0\.1\] - 2026-05-20', multiLine: true));
      ...
      expect(
        ...
        isTrue,
        reason:
            '[Unreleased] must appear ABOVE [0.0.1] - 2026-05-20 per Keep a Changelog convention',
      );
    });
