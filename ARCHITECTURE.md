...
**`q<N>` (re-resolvable)**: minted at `dusk:find` and `dusk:observe` time. Stores a predicate (`text` / `semanticsLabel` / `keyValue`) and re-walks the live tree on every action. Playwright Locator pattern. UI changes trigger a `DuskStaleHandleException`.

A `dusk:observe` handle carries one extra field, `DuskQuery.matchIndex`, and the two mint sites differ for a reason. A `dusk:find` predicate is CHOSEN, so answering the first match and reporting the total (`label 'X' matched N nodes; refine with ...`) puts the next move where it belongs. An observe predicate is DERIVED from a node the walk already holds, and observe promises one candidate per interactive node, so without an index every candidate sharing a label resolves to the first and an agent acting on the third row moves the first with both calls reporting success. The index is the node's position among every node whose label matched, in walk order, counted before observe's role filter and before its interactivity check so the two walks count the same universe. Playwright's `locator.nth(i)` is the same pair. An index that no longer resolves is a no-match rather than a fall back to a surviving namesake.
