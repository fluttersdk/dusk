...
- `ArtisanBoot.connected` (32 commands): requires a running app's VM Service URI from `~/.artisan/state.json`. Used for every `ext.dusk.*` route.
...
The 36 commands are registered in order at `lib/src/dusk_artisan_provider.dart:68-133` inside `commands()`. The 35 MCP descriptors live at `lib/src/dusk_artisan_provider.dart:136-1714` inside `mcpTools()`. `dusk_evaluate` is intentionally MCP-only (no CLI mirror) because `magic_tinker` owns the connected REPL surface; never duplicate it under `dusk:`.
